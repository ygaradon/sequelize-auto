    ...
    .success(function(tables){
      ...
      async.each(tables, function(table, _callback){
        ...
        async.each(tableNames, function(table, _callback){
          ...
          var models = "models";
          text[models] = "/* jshint indent: " + options.indentation + " */\n\n";
          text[models] += "module.exports = function(sequelize, DataTypes) {\n";
          text[models] += spaces + "var self = { \n";
          text[models] += Object.keys(imports).map(function(i){return imports[i];}).join('');
          text[models] += spaces + "};\n";
          text[models] += Object.keys(relations).map(function(i){return relations[i];}).join('');
          text[models] += spaces + "return self;\n";
          text[models] += "};\n";
